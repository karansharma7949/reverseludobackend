...
CREATE TABLE IF NOT EXISTS inventory (
  ...
  item_images JSONB NOT NULL, -- Multiple images for different contexts
...
INSERT INTO inventory (item_id, item_name, item_type, item_images, item_price) VALUES
  -- DICE: Needs thumbnail, preview, and all animation frames (01.png to 15.png) + result faces (dice1-6.png)
  ('classic_dice', 'Classic Dice', 'dice', 
   ...
   0),
  -- BOARD: Needs thumbnail, preview, and the actual board image
  ('classic_board', 'Classic Board', 'board', 
   ...
   0),
  -- TOKEN: Needs thumbnail, preview, and separate images for each team color
...
DROP POLICY IF EXISTS "Anyone can view inventory items" ON inventory;
DROP POLICY IF EXISTS "Only admins can modify inventory" ON inventory;
...
-- (You can adjust this based on your admin setup)
...
COMMENT ON COLUMN inventory.item_images IS 'JSONB object with multiple image URLs for different contexts (thumbnail, preview, in_game, etc.)';
