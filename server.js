...
//   // For 2-player mode, use diagonal positions (red and yellow)
//   // For 3-player mode, use red, green, yellow (skip blue)
//   // For 4-player mode, use all colors
//   let availableColors;
//   if (noOfPlayers === 2) {
//     availableColors = ['red', 'yellow']; // Diagonal positions
//   } else if (noOfPlayers === 3) {
//     availableColors = ['red', 'green', 'yellow']; // Skip blue for better spacing
//   } else {
...
//     console.log('Starting game with first turn to real player:', firstTurnPlayer);
...
// // Helper function to find room in game_rooms or tournament_rooms
// async function findGameRoom(roomId) {
//   // First try game_rooms
//   let { data: gameRoom, error: fetchError } = await supabaseAdmin
//     .from('game_rooms')
//     .select('*')
//     .eq('room_id', roomId)
//     .single();
...
//   if (!tournamentError && tournamentRoom) {
...
//     // Get game room from either table
//     const { gameRoom, tableName } = await findGameRoom(roomId);

//     if (!gameRoom) {
//       console.log('Game room not found in any table');
//       return res.status(404).json({ error: 'Game room not found' });
//     }
    
//     console.log('Found room in table:', tableName);
...
//     // Final position is 61 (home center) - tokens in home column (57-60) can still move
//     const finalPosition = 61;
...
//         // Token already at final position (61) - cannot move
//         continue;
//       } else {
//         // Token is on the board or in home column (1-60)
//         // Can move if the new position doesn't exceed final position (61)
...
//     // This prevents the bug where counter persists across turns
//     const consecutiveSixes = gameRoom.consecutive_sixes || {};
//     if (nextTurn !== userId) {
//       // Turn is passing to another player - reset current player's consecutive sixes
//       consecutiveSixes[userId] = 0;
//     }
